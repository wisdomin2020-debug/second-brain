...
export const resurfaceStaleIdeas = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    const suggestions = await step.run("generate-suggestions", async () => {
      ...
      for (const idea of staleIdeas) {
        const { text } = await generateText({
          ...
          prompt: `The user had this idea a while ago: "${idea.content}". 
                   Can you give a quick, punchy suggestion or a question to help them continue or execute on this?`,
        });
    ...
    // In a real app, you might send an email or a push notification here.
    // For now, we'll just log it or maybe insert into a 'notifications' table.
    return { suggestions };
  }
);
